...
def load_data(embedding_dir,training_cases_dir,step_length,embedding_length):
...
#    training_data_number=0
    ...
    while line:
        ...
        for i in range(0,len(sline),2):
            if (i/2%2) == 0:
                X=np.append(X,copy.deepcopy(entity2vec[int(sline[i+1])]),axis=0)

            elif (i/2%2) == 1:
                X=np.append(X,copy.deepcopy(relation2vec[int(sline[i+1])]),axis=0)
...
def create_model(step_length,embedding_length,neurons):
    # create your model using this function
    ## drug-p-e-p-target-p-e-p-disease
    # define LSTM configuration
    # create LSTM
    model = Sequential()
    Dropout(0.2, input_shape=(step_length, embedding_length))
    model.add(Dropout(0.5,input_shape=(step_length, embedding_length)))
...
if __name__ == "__main__":
    print("embedding length is: 25 50 75 100?")
    embedding_length=input()
    embedding_length=int(embedding_length)
    print("neurons are:")
    neurons=input()
    neurons=int(neurons)
    X, y = load_data("/home/a914/zhaodi/Embedding/Random_Embedding/"+str(embedding_length)+"/","/home/a914/zhaodi/LBD_paper_GraphEmbedding_LSTM/data/TrainingCases/",9,embedding_length)
    output_result=open("/home/a914/zhaodi/LBD_paper_GraphEmbedding_LSTM/data/results/RandomEmbedding/"+str(embedding_length)+"_"+str(neurons)+".txt","w+")
    training_model_file="/home/a914/zhaodi/LBD_paper_GraphEmbedding_LSTM/data/Training_Models/LSTM_RandomEmbedding/"
    ...
    for cross_validation_number in range(10):
        ...
        model = create_model(step_length,embedding_length,neurons)
        ...
        output_result.write("\n")
        model.save(training_model_file+str(embedding_length)+"_"+str(neurons)+"_"+str(cross_validation_number)+".h5")
    output_result.close()
